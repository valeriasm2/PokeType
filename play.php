...
// Permadeath
$isPermadeath = !empty($_SESSION['permadeath']);
...
// Cargar frases desde JSON por idioma
...
// Número de frases según dificultad
...
<title>Poketype - <?= htmlspecialchars($tPlay['title'] ?? 'Juego') ?></title>
...
<div id="lives-container"><div id="lives-box"></div></div>
...
<div id="container">
    <h1>Poketype</h1>
    <p><?= $tPlay['difficulty_label'] ?>: <strong><?= $tIndex["difficulty_{$difficulty}"] ?? ucfirst($difficulty) ?></strong></p>
...
<div id="combo-popup" role="status" aria-live="polite" style="display:none;">
    <div>
        <span class="label"><?= htmlspecialchars($tPlay['combo_label'] ?? 'Combo') ?></span>
        <span class="mult" id="combo-mult">x1</span>
        <div id="combo-timer" style="display:block;"><div id="combo-timer-bar"></div></div>
    </div>
</div>

<script>
const frasesData = <?= json_encode($frasesSeleccionadas) ?>;
const tPlayData = <?= json_encode($tPlay) ?>;
const bonusDificultad = <?= $bonus ?>;
const permadeathEnabled = <?= $isPermadeath ? 'true' : 'false' ?>;
const playerName = <?= json_encode($name) ?>;

let fraseIndex=0, charIndex=0, estado=[], puntosTotales=0, totalHits=0, totalTimeBonus=0, comboLevel=1;
...
function modificarVidas(){
    ...
    if(!permadeathEnabled){
        ...
        let hearts=''; for(let i=0;i<5;i++) hearts+='♡';
        livesBox.innerText = hearts + ' (' + (tPlayData.permadeath_off ?? 'permadeath off') + ')';
        return;
    }
    livesBox.classList.remove('disabled');
    let hearts=''; for(let i=0;i<vidas;i++) hearts+='❤';
    for(let i=vidas;i<5;i++) hearts+='♡';
    livesBox.innerText = hearts + (vidas<=2 ? ' ⚠️ '+vidas+' ' + (tPlayData.lives_left ?? 'vidas') : '');
}
modificarVidas();

function perderVidasEff(){ livesBox.classList.add('life-lost'); setTimeout(()=>livesBox.classList.remove('life-lost'),400); }

function normalizar(char){ return char.normalize("NFD").replace(/[\u0300-\u036f]/g,""); }

let startTimeGlobal=null;
function startGlobalTimer(){
    startTimeGlobal=Date.now();
    setInterval(()=>{ document.getElementById("timer").textContent = ((Date.now()-startTimeGlobal)/1000).toFixed(2); },10);
}
...
function jugar(e){
    ...
    if(acertado){
        correctSound.play();
        correctSinceLevel++;
        consecutiveWrong=0; singleWrongPending=false;
        ...
        if(permadeathEnabled){ vidas=Math.max(0,vidas-1); perderVidasEff(); modificarVidas(); if(vidas<=0){ document.removeEventListener("keydown", jugar); fraseEl.innerHTML+="<br><br><strong>💀 "+(tPlayData.permadeath_out??"Permadeath: no quedan vidas.")+"</strong>"; setTimeout(()=>enviarScore(puntosTotales,true),1500); return;}}
    }
    ...
    if(charIndex===frase.length){
        ...
        fraseEl.innerHTML+="<br><br><strong>"+(tPlayData.phraseCompleted??"✅ ¡Frase completada!")+"</strong>";
        ...
        if(fraseIndex===frasesData.length) setTimeout(()=>enviarScore(puntosTotales,false),1200);
        else setTimeout(()=>iniciarCuentaAtras(),1200);
    }
}

// Enviar score
function enviarScore(finalScoreBase, muerto=false){
    const finalScore=Math.floor((finalScoreBase+bonusDificultad)*comboLevel);
    const form=document.createElement("form");
    form.method="POST"; form.action="gameover.php";
    form.innerHTML=`
...
function iniciarCuentaAtras() {
    ...
    progressText.innerText = tPlayData.progress
        .replace("%d", fraseIndex + 1)
        .replace("%d", frasesData.length);
    progressBar.style.width = (fraseIndex / frasesData.length) * 100 + "%";

    let intervalo = setInterval(() => {
        let current = parseInt(countdownEl.innerText);
...
document.addEventListener("keydown",(e)=>{ if(e.key==="Escape"){ buttonSound.play(); setTimeout(()=>document.getElementById("back-btn").click(),200);} });
</script>
</body>
</html>
